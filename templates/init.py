...
def changeFlightStatus():
	...
	#update flight if it exists
        ...
        '''
        cursor.execute(query, (newStatus, airlineName, flightNumber, depDate, depDateEnd))
        conn.commit()
        result=cursor.fetchone()
        error=None
...
def addAirplaneForm():
    ...
    if request.method == 'POST':
        ...
        VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
        cursor.execute(query, (airline_name, airplane_id, number_of_seats, manu_company, model_number, manu_date, age))
        conn.commit()
...
@app.route('/viewFlightRatingsForm', methods=['GET'])
def viewFlightRatingsForm():
    # Check if the user is logged in and is an authorized airline staff
    ...
    # Retrieve flight ratings and comments
    cursor = conn.cursor()
    query = '''
        SELECT f.airline_name, f.flight_number, AVG(e.rating) as average_rating, GROUP_CONCAT(e.comments SEPARATOR ', ') as comments
        FROM Flight f
        LEFT JOIN Evaluations e ON f.airline_name = e.airline_name AND f.flight_number = e.flight_number AND f.departure_date_time = e.departure_date_time
        GROUP BY f.airline_name, f.flight_number
    '''
    cursor.execute(query)
    flight_ratings = cursor.fetchall()
    cursor.close()

    return render_template('viewFlightRatingsForm.html', flight_ratings=flight_ratings)

@app.route('/scheduleMaintenanceForm', methods=['GET'])
def scheduleMaintenanceForm():
	pass

@app.route('/viewEarnedRevenueForm', methods=['GET'])
def viewEarnedRevenueForm():
	pass


@app.route('/viewFrequentCustomersForm', methods=['GET'])
